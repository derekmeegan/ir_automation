            ...
            async with async_playwright() as p:
                ...
                if self.refine_link_list or not candidate_elements:
                    if candidate_elements:
                        candidates = []
                        keywords = [kw.lower() for kw in self.verify_keywords if kw]
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are an assistant that extracts key financial metrics from text, "
                        "including forward guidance. Return only valid JSON with fields for "
                        "eps, net_sales, operating_income, net_income, forward_guidance, and any other "
                        "relevant metrics you find. For forward guidance, capture any references "
                        "to future revenue or earnings projections."
                    )
                },
                {
                    "role": "user",
